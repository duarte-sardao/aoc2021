...
vector<int> repeatFive(vector<int> line) {
    vector<int> newl = line;
    for(int i = 1; i <= 4; i++) {
        for(int val: line) {
            newl.push_back(updateval(val, i));
        }
    }
    return newl;
}

vector<vector<int>> repeatArr(vector<vector<int>> a) {
    vector<vector<int>> newa = a;
    for(int i = 1; i <= 4; i++) {
        for(auto vec: a) {
            vector<int> newv;
            for(auto val: vec) {
                newv.push_back(updateval(val, i));
            }
            newa.push_back(newv);
        }
    }
